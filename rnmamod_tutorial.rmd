...
As you may have correctly guessed, **exclusion** and **imputation** are the bad 
and ugly methods, respectively, to handle missing participants. **Exclusion** is 
illustrated in [Figure 2](#fig:excluded): missing participants in both interventions 
...
increasing the risk to estimate a biased intervention effect and yield a spuriously 
...
The **pattern-mixture model** circumvents the limitations of exclusion and imputation 
by adjusting, rather than fixing, the average outcome (here, the risk of event) 
to the assumed missingness mechanism, and propagating this assumption to the standard 
error of the estimated intervention effect, while maintaining the randomised sample. 
Therefore, the pattern-mixture model is rightfully entitled as the good method 
to handle missing participants. [Figure 4](#fig:imor) illustrates two normal 
distributions of the **informative missingness odds ratio (IMOR)**, a central 
parameter in the pattern-mixture model for a binary outcome, that quantifies the 
ratio of the odds of an event in missing participants to the odds of an event in 
participants completing the trial (completers). IMOR equal to 1 coincides with 
the MAR mechanism, and IMOR different from 1 implies the MNAR mechanism. We have 
considered a different IMOR distribution for each intervention: the first intervention 
is assumed to have on average twice the odds of an event in the completers 
than that in missing participants (the mode), with other scenarios being less and 
less likely to occur as we deviate from the mode. On the contrary, the second 
intervention is assumed to have on average twice the odds of an event in missing
participant than that in completers (the mode), again, with other scenarios being 
less and less likely to occur as we deviate from the mode.

```{r imor, echo=FALSE, out.width = "50%", out.height = "15%", fig.align="center", fig.cap = "Figure 4: The IMOR parameter under different scenarions for each intervention", fig.alt=""}
knitr::include_graphics("imor.png")
```
