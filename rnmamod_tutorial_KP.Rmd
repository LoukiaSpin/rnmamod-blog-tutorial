...
The `nma.baker2009` dataset was selected due to its high number of missing participants in most trials, interventions and comparisons. Use the `heatmap_missing_dataset` function to view the distribution of missing participants (in percent) in the dataset. Most trials have interventiones with high level of missingness, with only a few trials having zeror or low missingness rate.
...
We have created the functions `run_series_meta` and `series_meta_plot` to help us explore the implications of assumin consistency and common heterogeneity, as opposed to performing pairwise meta-analysis separately for each observed comparison in the network.

First, we use the `run_series_meta` function to conduct separate pairwise meta-analyses for all observed comparisons in the network:
