...
Without further ado, let's run a Bayesian random-effects (`model = "RE"`) network meta-analysis while, at the same time, addressing the missing participants in each intervention arm of every trial. The effect measure of interest is odds ratio (`measure = "OR"`). We consider a half-normal prior distribution on the between-trial standard deviation with scale parameter equal to 1 (`model = "RE"`). We will estimate one IMOR parameter for each intervention in the network (`heter_prior = list("halfnormal", 0, 1)`) under the *MAR* assumption on average (`mean_misspar = c(0, 0)`) with a variance equal to 1 (`var_misspar = 1`). The investigated outcome is harmful (`D = 0`). The following code :
