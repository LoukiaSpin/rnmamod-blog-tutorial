...
Here, the function inherits the arguments `n_chains`, `n_iter`, `n_burning`, and `n_thin` from the `run_model` function via `res`; however, we can also specify these arguments anew. Like with `run_model`, the console prints the progress of simulations together with the number of models to run: there are 15 observed comparisons in our network; hence, the console prints this message (in red), followed by a progress bar for each analysis alongside the model update until convergence:
...
Next, insert `meta` in the function `series_meta_plot` to experience the visualisation toolkit for this analysis:
