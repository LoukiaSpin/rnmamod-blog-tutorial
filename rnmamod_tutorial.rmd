...
The `rankosucra_plot` function creates an plot that integrates the rankograms and SUCRA curves of all interventions in the network:
...
The interventions are sorted from the best to the worst based on their SUCRA value (the number on the top left corner of each panel). The `rankosucra_plot` function can also display the hierarchy results from two outcomes. , Type `?rankosucra_plot` for more details.